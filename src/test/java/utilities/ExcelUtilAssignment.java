...
public class ExcelUtilAssignment {
	...
	public static List<Map<String, String>> readData(String fileName, String sheetName) {
		...
		try {
			...
			for (int r = 1; r < totalRows; r++) {
				...
				for (int c = 0; c < totalCloumns; c++) {
					...
					String columnName = sh.getRow(0).getCell(c).getStringCellValue(); // Store the key value
																						// (Header/ColumnName[A1])
					String columnValue = sh.getRow(r).getCell(c).getStringCellValue(); // Store the Value (Data)[A2]

					rowData.put(columnName, columnValue); // Store the Data in Map in Key Value pair[A1(Product
															// ID),A2(Value)]
				}
		...
		return data; // Send the Data to List Method
